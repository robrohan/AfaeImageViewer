...
/**
 * An integrated Image viewer, defined as an editor to make
...
public class ImageViewEditor extends EditorPart {
	...
	{
		//parent.setBackground(new Color((Device)parent.getDisplay(),91,91,91));
		...
		{
			DimLabelValue.setText(imageCanvas.getImageData().width + "x" + imageCanvas.getImageData().height);
			CDepthLabelValue.setText(imageCanvas.getImageData().depth  + "");
			//SizeLabelValue.setText(imageCanvas.getImageData().data.length +" Bytes");
		}
		else
		{
	...
	protected void closeEditor() {
		Display.getDefault().asyncExec(new Runnable() {
			public void run() {
				getEditorSite().getPage().closeEditor(ImageViewEditor.this, false);
			}
		});
	}
