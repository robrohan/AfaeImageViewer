...
import java.awt.geom.AffineTransform; //import java.io.File;
...
import org.eclipse.swt.events.SelectionEvent; //import org.eclipse.swt.graphics.Color;
...
import org.eclipse.swt.widgets.Composite; //import org.eclipse.swt.widgets.FileDialog;
...
public class SWTImageCanvas extends Canvas {
	...
	static final float ZOOMIN_RATE = 2.0f; /* zoomin rate */
	static final float ZOOMOUT_RATE = 0.5f; /* zoomout rate */
	...
	private void paint(GC gc) {
		...
		if (sourceImage != null) {
			...
			newGC.setBackground(getShell().getDisplay().getSystemColor(
					SWT.COLOR_GRAY));
			newGC.setForeground(getShell().getDisplay().getSystemColor(
					SWT.COLOR_GRAY));
			newGC.fillGradientRectangle(0, 0, clientRect.width,
					clientRect.height, true);

			// fill the whole background with a gradient so the image background
			// can
			...
			// newGC.fillGradientRectangle(0, 0, clientRect.width,
			// clientRect.height, true);
			newGC.fillGradientRectangle(destRect.x - 1, destRect.y - 1,
					destRect.width + 1, destRect.height + 1, true);

			newGC.setForeground(getShell().getDisplay().getSystemColor(
					SWT.COLOR_BLACK));
			// draw a littl box around the image
			newGC.setLineStyle(SWT.LINE_DASHDOTDOT);
			newGC.drawRectangle(destRect.x - 1, destRect.y - 1,
	...
	private String getFallbackImage() {
		try {
			URL installURL = ImageViewerPlugin.getDefault().getBundle()
					.getEntry("/");
			// URL snipsdir = Platform.resolve(new URL(installURL,
			// FALLBACK_IMAGE));
			URL snipsdir = FileLocator.resolve(new URL(installURL,
					FALLBACK_IMAGE));
	 ...
	 * if it can't load the passed image so this is the only way to tell if the
	 * load went ok.
	 * 
	 * @return
	 */
